"use client"
import React from 'react'
import Link from 'next/link'
import { Plus, Home, Search, BookOpen, Crown } from 'lucide-react'
import { useSession } from 'next-auth/react'
import { usePathname } from 'next/navigation'
import { useLanguage } from '../providers/LanguageProvider'
import { useTheme } from '../providers/ThemeProvider'

export default function Footer() {
  const pathname = usePathname()
  const { status } = useSession()
  const { t } = useLanguage()
  const { isDark } = useTheme()

  if (status !== "authenticated") {
    return null
  }

  return (
    /* Background: #121212 | Height: 64px | Border: #262626 */
    <footer className={`fixed bottom-0 left-0 right-0 h-[64px] border-t z-50 flex items-center justify-around px-4 ${isDark ? 'bg-[#1c1c1e] border-[#262626]' : 'bg-white border-[#E2E4E9]'}`}>
      
      {/* Left Icon - Home */}
      <Link 
        href="/cards/learning" 
        className={`flex flex-col items-center justify-center transition-colors ${
          pathname === '/cards/learning' ? 'text-[#3B82F6]' : isDark ? 'text-[#9CA3AF] hover:text-[#FFFFFF]' : 'text-[#6B7280] hover:text-[#111827]'
        }`}
      >
        <Home size={19} />
        <span className="text-[11px] font-bold uppercase tracking-widest mt-1">{t('footer.home')}</span>
      </Link>

      {/* Left-Center Icon - Library */}
      <Link 
        href="/cards/library" 
        className={`flex flex-col items-center justify-center transition-colors ${
          pathname === '/cards/library' ? 'text-[#3B82F6]' : isDark ? 'text-[#9CA3AF] hover:text-[#FFFFFF]' : 'text-[#6B7280] hover:text-[#111827]'
        }`}
      >
        <BookOpen size={19} />
        <span className="text-[11px] font-bold uppercase tracking-widest mt-1">{t('footer.library')}</span>
      </Link>

      {/* Center Primary Action Button */}
      <div className="relative -top-5">
        <Link
          href="/cards/new"
          className={`flex items-center justify-center w-14 h-14 bg-[#3B82F6] text-[#FFFFFF] rounded-full shadow-[0_4px_20px_rgba(59,130,246,0.4)] hover:bg-[#1D4ED8] active:scale-95 transition-all border-4 ${isDark ? 'border-[#121212]' : 'border-white'}`}
        >
          <Plus size={28} strokeWidth={3} />
        </Link>
      </div>

      {/* Right-Center Icon - Search */}
      <Link 
        href="/cards/search" 
        className={`flex flex-col items-center justify-center transition-colors ${
          pathname === '/cards/search' ? 'text-[#3B82F6]' : isDark ? 'text-[#9CA3AF] hover:text-[#FFFFFF]' : 'text-[#6B7280] hover:text-[#111827]'
        }`}
      >
        <Search size={19} />
        <span className="text-[11px] font-bold uppercase tracking-widest mt-1">{t('footer.search')}</span>
      </Link>

      {/* Right Icon - Profile */}
      <Link 
        href="/profile" 
        className={`flex flex-col items-center justify-center transition-colors ${
          pathname === '/profile' ? 'text-[#3B82F6]' : isDark ? 'text-[#9CA3AF] hover:text-[#FFFFFF]' : 'text-[#6B7280] hover:text-[#111827]'
        }`}
      >
        <Crown size={19} />
        <span className="text-[11px] font-bold uppercase tracking-widest mt-1">{t('footer.profile')}</span>
      </Link>

    </footer>
  )
}