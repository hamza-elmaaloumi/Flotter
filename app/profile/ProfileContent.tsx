    ...
    <div dir={language === 'ar' ? 'rtl' : 'ltr'} className={`min-h-screen antialiased pb-[64px] ${isDark ? 'bg-[#121212] text-[#FFFFFF]' : 'bg-[#F8F9FA] text-[#111827]'}`}>
      ...
      <div className="max-w-[340px] md:max-w-md mx-auto px-[6px] pt-16 relative z-10">
        ...
        {isEditing ? (
          <section className={`mb-[24px] border p-6 rounded-[24px] shadow-2xl ${isDark ? 'bg-[#1C1C1E] border-[#2D2D2F]' : 'bg-white border-[#E2E4E9]'}`}>
            <div className="flex items-center justify-between mb-8">
              ...
              <Link 
                href="/profile" 
                className={`w-10 h-10 rounded-full border flex items-center justify-center transition-all ${isDark ? 'bg-[#222222] border-[#2D2D2F] text-[#6B7280] hover:text-white' : 'bg-[#F0F1F3] border-[#E2E4E9] text-[#9CA3AF] hover:text-[#111827]'}`}
              >
                <X size={18} />
              </Link>
            </div>
            <ProfileForm user={user} isDark={isDark} />
          </section>
        ) : (
          <section className="flex flex-col items-center mb-[24px]">
            <div className="relative mb-4">
              <div className={`w-20 h-20 rounded-[12px] border overflow-hidden flex items-center justify-center shadow-2xl ${isDark ? 'bg-[#222222] border-[#2D2D2F]' : 'bg-[#F0F1F3] border-[#E2E4E9]'}`}>
                {user.image ? (
              ...
              <div className={`absolute -bottom-1 -right-1 bg-[#10B981] p-1.5 rounded-[8px] border-[3px] ${isDark ? 'border-[#121212]' : 'border-[#F8F9FA]'}`}>
                <Fingerprint size={12} className="text-[#000000]" />
              </div>
            </div>
            <h1 className="text-[19px] font-bold tracking-tight">{user.name ?? t('profile.learner')}</h1>
            <p className={`text-[12px] font-medium mb-4 ${isDark ? 'text-[#9CA3AF]' : 'text-[#6B7280]'}`}>{user.email}</p>
            <Link 
              href="/profile?edit=true" 
              className={`px-5 py-2 rounded-full border text-[11px] font-bold uppercase tracking-widest text-[#3B82F6] transition-colors ${isDark ? 'bg-[#1C1C1E] border-[#2D2D2F] hover:bg-[#222222]' : 'bg-white border-[#E2E4E9] hover:bg-[#F0F1F3]'}`}
            >
        ...
        <div className={`border p-4 rounded-[16px] mb-[20px] ${isDark ? 'bg-[#1C1C1E] border-[#2D2D2F]' : 'bg-white border-[#E2E4E9]'}`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`text-[16px] font-bold ${isDark ? 'text-[#FFFFFF]' : 'text-[#111827]'}`}>{effectiveMonthlyXp.toLocaleString()} <span className="text-[11px] text-[#6B7280] font-normal">{t('profile.xpThisMonth')}</span></p>
            </div>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${
                user.isPro
                  ...
                  : isDark ? 'bg-[#222222] text-[#6B7280] border border-[#2D2D2F]' : 'bg-[#F0F1F3] text-[#6B7280] border border-[#E2E4E9]'
              }`}>
        ...
        <div className={`border rounded-[16px] overflow-hidden mb-6 ${isDark ? 'bg-[#1C1C1E] border-[#2D2D2F]' : 'bg-white border-[#E2E4E9]'}`}>
          ...
          <div className={`divide-y ${isDark ? 'divide-[#262626]' : 'divide-[#EBEDF0]'}`}>
            <Link 
              href="/profile?edit=true" 
              className={`w-full h-[56px] flex items-center justify-between px-5 transition-all group ${isDark ? 'hover:bg-[#222222]' : 'hover:bg-[#F0F1F3]'}`}
            >
              <div className="flex items-center gap-3">
                ...
                <span className={`text-[14px] font-regular ${isDark ? 'text-[#FFFFFF]' : 'text-[#111827]'}`}>{t('profile.editProfile')}</span>
              </div>
              <ChevronRight size={14} className="text-[#6B7280]" />
            </Link>

            <button className={`w-full h-[56px] flex items-center justify-between px-5 transition-all group ${isDark ? 'hover:bg-[#222222]' : 'hover:bg-[#F0F1F3]'}`}>
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-[8px] border flex items-center justify-center ${isDark ? 'bg-[#121212] border-[#2D2D2F]' : 'bg-[#F0F1F3] border-[#E2E4E9]'}`}>
                  <Mail size={16} className="text-[#9CA3AF] group-hover:text-[#3B82F6] transition-colors" />
                </div>
                <span className={`text-[14px] font-regular ${isDark ? 'text-[#FFFFFF]' : 'text-[#111827]'}`}>{t('profile.emailAddress')}</span>
              </div>
              <ChevronRight size={14} className="text-[#6B7280]" />
            </button>
            ...
            <button className={`w-full h-[56px] flex items-center justify-between px-5 transition-all group ${isDark ? 'hover:bg-[#222222]' : 'hover:bg-[#F0F1F3]'}`}>
              <div className="flex items-center gap-3">
                ...
                <span className={`text-[14px] font-regular ${isDark ? 'text-[#FFFFFF]' : 'text-[#111827]'}`}>{t('profile.security')}</span>
              </div>
              <ChevronRight size={14} className="text-[#6B7280]" />
            </button>
          </div>
        ...
        <div className="mt-[20px] text-center">
          <div className={`inline-block px-3 py-1 rounded-full border ${isDark ? 'border-[#2D2D2F] bg-[#1C1C1E]' : 'border-[#E2E4E9] bg-white'}`}>
